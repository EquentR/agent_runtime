...
func TestApplyStreamEvent_OutputItemDoneUpdatesImageGenerationResult(t *testing.T) {
	stats := &model.StreamStats{ResponseType: model.StreamResponseUnknown}
	acc := newStreamToolCallAccumulator()
	outputItems := make([]responses.ResponseOutputItemUnion, 0)
	reasoningItems := make(map[int64]model.ReasoningItem)
	...
	// Simulate: output_item.added with initial image_generation_call (no result yet)
	applyStreamEvent(responses.ResponseStreamEventUnion{
		Type:        "response.output_item.added",
		OutputIndex: 0,
		Item:        responses.ResponseOutputItemUnion{Type: "image_generation_call", ID: "ig_1", Status: "in_progress"},
	}, acc, &outputItems, reasoningItems, stats, &once, start, splitter, &reasoning,
		func(string) {}, func(string) {}, func(model.StreamEvent) {}, func(string) {}, func(error) {})

	if len(outputItems) != 1 || outputItems[0].Result != "" {
		t.Fatalf("after output_item.added: result = %q, want empty", outputItems[0].Result)
	}

	// Simulate: output_item.done with final image result
	applyStreamEvent(responses.ResponseStreamEventUnion{
		Type:        "response.output_item.done",
		OutputIndex: 0,
		Item:        responses.ResponseOutputItemUnion{Type: "image_generation_call", ID: "ig_1", Result: "base64imagedata", Status: "completed"},
	}, acc, &outputItems, reasoningItems, stats, &once, start, splitter, &reasoning,
		func(string) {}, func(string) {}, func(model.StreamEvent) {}, func(string) {}, func(error) {})

	if len(outputItems) != 1 {
		t.Fatalf("outputItems length = %d, want 1", len(outputItems))
	}
	if outputItems[0].Result != "base64imagedata" {
		t.Fatalf("after output_item.done: result = %q, want base64imagedata", outputItems[0].Result)
	}
	if outputItems[0].Status != "completed" {
		t.Fatalf("after output_item.done: status = %q, want completed", outputItems[0].Status)
	}
}
