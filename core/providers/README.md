...
- continuation 请求保留 `tools`，使模型在工具结果回填后仍可继续发起后续工具调用。
- ProviderState 保存完整 Responses output archive，并在历史重放时恢复 reasoning、assistant message 与 function call；仅有 refs-only 状态时会回退到标准 assistant/tool 消息重放，不再依赖 `item_reference`。
...
- `previous_response_id` 在当前验证目标上不可作为生产级 continuation 机制；适配器继续使用显式历史和 ProviderState output replay。
