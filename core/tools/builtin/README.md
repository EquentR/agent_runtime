...
所有文件类工具的操作范围限制在工作目录内，不支持符号链接和 `..` 路径穿越。工具也会拒绝写入内部 workspace 元数据 sidecar（当前包括 `.workspace-state.json` 与 `.workspace-baseline.json`），避免模型或用户任务篡改合并状态与 baseline manifest。
