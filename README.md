...
一个基于 Go 的轻量级 Agent Runtime，用于承载 LLM provider、tool calling、MCP、memory 与基础服务装配能力。当前仓库更接近可运行的 runtime skeleton，加上一个基于 Gin + SQLite 的示例应用，而不是完整的 agent product。

## 当前状态

- 已落地的核心能力集中在 `core/providers`、`core/tools`、`core/mcp`、`core/memory`、`core/types` 与 `pkg/*`
- 当前唯一命令行入口是 `cmd/example_agent`
- 示例应用会读取 `conf/app.yaml`，展开环境变量后启动 HTTP 服务、SQLite 和 migration
...
- **Example App**：`app/*` 下提供最小可运行的服务装配、路由、handler、logic 与 migration 示例
...
- `pkg/rest.Config` 使用的字段名是 `staticPaths`，而 `conf/app.yaml` 示例目前仍写成 `staticPath`；如果要依赖静态资源配置，先统一这一处
